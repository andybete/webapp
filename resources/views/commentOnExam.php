...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['send']) && isset($_POST['check'])) {
        ...
        foreach ($selectedques as $key => $status) {
            ...
            if ($status === 'checked') {
                $comment = isset($_POST['comment'][$department][$level][$ques_no]) ? $_POST['comment'][$department][$level][$ques_no] : '';

                $updateSql = "UPDATE question SET comment = '$comment' WHERE department = '$department' AND level = '$level' AND ques_no = '$ques_no'";
    ...
    <?php if (mysqli_num_rows($result) > 0) : ?>
        ...
        <form method="POST">
            <?php foreach ($groupedData as $department => $sectorData) : ?>
                        ...
                        <tbody>
                            <?php foreach ($levelData as $row) : ?>
                                ...
                                <tr class="<?php echo $rowClass; ?>">
                                    ...
                                    <td><?= $row['answer']; ?></td>
                                    <td>
                                        <input type="text" name="comment[<?= $department ?>][<?= $level ?>][<?= $ques_no ?>]" placeholder="Enter comment">
                                    </td>
               ...
               <?php endforeach; ?>
            <?php endforeach; ?>
			<button type="submit" name="send" class="submit-button">Send comment</button>
