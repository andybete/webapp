...
// Check if the practical exam has been taken
$query = "SELECT practical_exam FROM approvecandidate WHERE username='$username' AND level >= 3";
$result = mysqli_query($conn, $query);
$row = mysqli_fetch_assoc($result);

if (empty($row['practical_exam']) && $level >= 3) {
echo "<script src='https://cdn.jsdelivr.net/npm/sweetalert2@11.1.2'></script>";
    echo "<script>
            Swal.fire({
                icon: 'error',
                title: 'Oops...',
                text: 'Please first take the practical exam.'
            }).then(function() {
...
$sql = "SELECT desired_date, desired_time FROM timer WHERE list_department='$department' AND level='$level'";
...
$sql = "SELECT hr, min FROM timer WHERE list_department='$department' AND level='$level'";
$result = $conn->query($sql);
